...
export default function Dashboard() {
  ...
  async function loadData() {
    try {
      const [weatherRes, todayRes, ddayRes, userRes, cardsRes] = await Promise.all([
  ...
  const quickLinks = [
    ...
    { label: '업무 계산기', icon: Zap, path: '/calculator', color: '#f59e0b', desc: '여비·수당·세금 자동 계산' },
    { label: '법인카드 관리', icon: CreditCard, path: '/card', color: '#ec4899', desc: '대여·반납 현황 관리' },
    { label: '분실물 관리', icon: Package, path: '/lost', color: '#f97316', desc: 'QR 생성 및 보관 현황' },
    { label: '자동화 센터', icon: Zap, path: '/automation', color: '#06b6d4', desc: 'PDF·엑셀·파일 자동화' },
  ]
  ...
  return (
    <div className="space-y-5 fade-in">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {quickLinks.map(({ label, icon: Icon, path, color, desc }) => (
            <button
              key={path}
